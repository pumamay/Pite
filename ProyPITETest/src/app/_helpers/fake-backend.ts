...
const usersKey = 'angular-crud-pite-users';
const usersJSON = localStorage.getItem(usersKey);
let users: any[] = usersJSON ? JSON.parse(usersJSON) : [{
  id: 1,
  firstName: 'Mariano',
  lastName: 'Ascencio',
  fullName: 'Mariano Ascencio',
  email: 'mascencio@pite.dev',
  password: 'Admin@123'
}];
...
export class FakeBackendInterceptor implements HttpInterceptor {
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    function basicDetails(user: any) {
      const { id, firstName, lastName, fullName, email, birthDay } = user;
      return { id, firstName, lastName, fullName, email, birthDay };
    }
